...
window.onload = function(){
	...
	var options = searchForm.searchField.opions;
 ...
 */
 function formSubmit() {
	...
	// 3. 폼 전송(요청)하기 - form의 action속성에 정의돈 url을 요청
	//						- 요청할 때는 폼안에 요소들을 파라메터로 전달.
	searchForm.submit();
...
<div class="input-group">
  <select class="form-select" name="searchField" id="inputGroupSelect04" aria-label="Example select with button addon">
	...
	<option value="content"
			${pageDto.cri.searchWord eq 'content' ? 'selected' : ''}>
			내용 </option>
  </select>
